class Autocomplete {
	...
	constructor(params) {
		...
		this.block = d.querySelector(`.${params.container}`);
		this.dropdown = this.block.querySelector('.autocomplete__dropdown');
		this.input = this.block.querySelector('.autocomplete-input');
		this.addListeners();
		return true;
	}
	...
	addListeners() {
		...
		_.input.addEventListener('change', function(event) {
			setTimeout(()=> {
				_.checkInput()
			},300)
		});
	...
	checkInput() {
		...
		if (_.input.dataset.selected === 'false') {
			...
			C.showError('Необходимо выбрать значение из выпадающего списка');
			_.input.value = '';
			this.hideList()
		}
	}
